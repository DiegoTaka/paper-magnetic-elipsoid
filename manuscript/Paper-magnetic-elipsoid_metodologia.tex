...
\Delta T \approx \hat{F}^T \bar{B}_i, \quad i=1,...,NF
\end{equation}

The field now $\bar{B}_i$ is represented this way to show that is in the geographic coordinates. One of the main steps to calculate the magnetic field of an ellipsoid is to change this system of coordinates to the body coordinates with origin in the center of it. The conversion can be done through a matrix of rotation $\bar{\bar{V}}$ that differs with the geometric type of ellipsoid, so the formulas will be shown later, separately for each one. But, in a general way, the conversion is:
\begin{equation}\label{eq:5}
\bar{r}_i = \bar{\bar{V}} \tilde{r}_i + \bar{r}_c,
\end{equation}

The vector $\bar{r}_i$ has the positions $x,y,z$ in the geographic coordinates, $\bar{r}_i$ has the positions $x1,x2,x3$ in the body coordinates and $\bar{r}_c$ is the position of the center of the body in the geographic coordinates.

The same way the matrix of conversion can be used to change the magnetic field calculate in the body coordinates, back to the geopraphic, where is more useful to us:
\begin{equation}\label{eq:5}
\bar{B}_i = \bar{\bar{V}} \tilde{B}_i
\end{equation}

The equation of the surface of an ellipsoid can be written in a matrix form as:
\begin{equation}
(\bar{\textbf{r}} - \bar{\textbf{r}}_c)^T \bar{\bar{A}} (\bar{\textbf{r}} - \bar{\textbf{r}}_c) = 1,
\end{equation}

and $\bar{\bar{A}}$ is a 3X3 matrix formed by:
\begin{equation}
\bar{\bar{A}} = \bar{\bar{V}} 
\left[\begin{matrix} 
\dfrac{1}{a^2+s} & 0 & 0 \\
...
where the columns of $\bar{\bar{V}}$ are the eigenvectors that represents the principal axes of the ellipsoid and the diagonal of the middle matrix the eigenvalues that represents the reciprocals of the square of the semi-axes $a^{-2}$, $b^{-2}$ and $c^{-2}$.
...
\begin{equation}
\textbf{v}_1 = (l_1,m_1,n_1) = (-\cos\alpha \cos\delta, -\sin\alpha \cos\delta, -\sin\delta)
\end{equation}

\begin{equation}
\begin{split}
\textbf{v}_2 = (l_2,m_2,n_2) = (\cos\alpha \cos\gamma \sin\delta + \sin\alpha \sin\gamma,\\ \sin\alpha \cos\gamma \sin\delta - \cos\alpha \sin\gamma, -\cos\gamma \cos\delta)
\end{split}
\end{equation}

\begin{equation}
\begin{split}
\textbf{v}_3 = (l_3,m_3,n_3) = (\sin\alpha \cos\gamma - \cos\alpha \sin\gamma \sin\delta,\\ -\cos\alpha \cos\gamma - \sin\alpha \sin\gamma \sin\delta, \sin\gamma \cos\delta)
\end{split}
\end{equation}
