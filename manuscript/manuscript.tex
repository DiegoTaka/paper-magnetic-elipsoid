...
orientation angles $\alpha$, $\beta$, and $\gamma$, which are not used
...
depending on the ellipsoid type.
...
peake1953, chang1961, macmillan1958, lowes1974,  clark1986, 
...
anisotropy of magnetic susceptibility, which is represented by the 
susceptibility tensor $\tilde{\mathbf{K}}$ and reflects the
preferred orientation of the magnetic minerals forming the
body, and the shape anisotropy, which is represented by the
depolarization tensor $\tilde{\mathbf{N}}^{\dagger}$ and
reflects the self-demagnetization associated to the body shape.
Notice that the resultant magnetization $\tilde{\mathbf{M}}$
(Eq. \ref{eq:M-tilde-remanence}) does not necessarily have
the same direction as the inducing field $\tilde{\mathbf{H}}_{0}$.
...
(Fig. \ref{fig:orientation-angles}a) can be approximated as follows:
\begin{equation}
...
is considered a good approximation for $\chi \leq 0.1$ SI.
...
(Eq. \ref{eq:H0}) and $\Delta \mathbf{B}(\mathbf{r}) = 
k_{m} \, \mathbf{V} \; \Delta \tilde{\mathbf{H}}(\tilde{\mathbf{r}})$,
with $\mathbf{V}$ defined according to the ellipsoid type and
$\Delta \tilde{\mathbf{H}}(\tilde{\mathbf{r}})$ defined by
...
the total-field anomaly $\Delta T(\mathbf{r})$
(Eqs. \ref{eq:delta-T} and \ref{eq:delta-T-approx}) 
are calculated according to Eq. \ref{eq:nij-tilde}, with 
