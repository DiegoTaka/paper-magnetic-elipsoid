...
Similarly, the transformation of a generic 
...
referred to a local coordinate system is given by
\begin{equation}
\tilde{\mathbf{M}} = \mathbf{V}^{\top} \mathbf{M} \mathbf{V} \: , 
\quad \tilde{\mathbf{p}} = \mathbf{V}^{\top} \mathbf{p} \: ,
\quad \tilde{\mathbf{d}} = \mathbf{V}^{\top} \mathbf{d} \: .
\label{eq:matrix_transformation}
\end{equation}
...
Let us start our description of the magnetic field produced
by ellipsoids by considering a magnetized ellipsoid immersed in
...
\mathbf{M}(\mathbf{r}^{\prime})^{\top} \left(
...
intensity at any point within the body and $\mathbf{K}$ is a 
...
\end{equation}
where $\mathbf{N}(\mathbf{r})$ is a $3 \times 3$ matrix whose
$\alpha \beta$-element $n_{\alpha \beta}(\mathbf{r})$ is given by
\begin{equation}
n_{\alpha \beta}(\mathbf{r}) = - 
\frac{1}{4\pi} \frac{\partial^{2}}{\partial \alpha \, \partial \beta}
\iiint_{V} 
\frac{1}{\| \mathbf{r} - \mathbf{r}^{\prime} \|}
\, dx^{\prime}dy^{\prime}dz^{\prime} \: , \quad \alpha = x, y, z \: , 
\quad \beta = x, y, z \: .
