...
These examples show the importance of the ellipsoidal
model in producing trustworthy geological models of
high-susceptibility orebodies, which may save significant 
cost associated with drilling.

As we can see, a vast literature about the magnetic
modelling of ellipsoidal bodies was developed in which 
are to be found the names of many brilliant researchers.
Nevertheless, interest in this subject has not yet died
out, as is evidenced by the list of modern papers in this
field.
...
To fill this gap, we provide here a set of routines to 
...
code.
It is hoped that the presentation of the theoretical and
practical aspects given here will be found useful to the
wide community of geoscientists.
...
In general, the susceptibility tensor $\mathbf{K}$ is represented,
in the main coordinate system, as follows:
\begin{equation}
\mathbf{K} = \mathbf{U}
\left[ \begin{array}{ccc}
k_{1} & 0 & 0 \\
...
\textit{principal directions} \citep{hrouda1982}.
For the particular case in which the principal directions 
coincide with the ellipsoid axes, the matrix $\mathbf{U}$ is
equal to the matrix $\mathbf{V}$ (Eqs. \ref{eq:V_triaxial_prolate}
and \ref{eq:V_oblate}), depending on the ellipsoid type.
...
macmillan1958, lowes1974,  peake1953, chang1961, clark1986, 
...
$\tilde{\mathbf{N}}^{\dagger}$ is a diagonal matrix
representing the transformed depolarization tensor 
evaluated within the volume $\vartheta$ of the ellipsoidal body.
For a given ellipsoid, the elements of $\tilde{\mathbf{N}}^{\dagger}$ 
are constants which are expressed in terms of the ellipsoid semi-axes.
These elements are presented in detail in a latter section.
...
In this case, the magnetization $\tilde{\mathbf{M}}$ (Eq. \ref{eq:M-tilde})
can be rewritten as follows:
\begin{equation}
\tilde{\mathbf{M}} = \left[ \begin{array}{ccc}
\frac{\chi}{1 - \chi \, \tilde{n}^{\dagger}_{11}} & 0 & 0 \\
0 & \frac{\chi}{1 - \chi \, \tilde{n}^{\dagger}_{22}} & 0 \\
0 & 0 & \frac{\chi}{1 - \chi \, \tilde{n}^{\dagger}_{33}}
\end{array} \right]
\tilde{\mathbf{H}}_{0} \: ,
\label{eq:M-tilde-K-isotropic}
\end{equation}
where $\tilde{n}^{\dagger}_{ii}$, $i = 1, 2, 3$, are the constant
elements of the transformed depolarization tensor
$\tilde{\mathbf{N}}^{\dagger}$.
Equation \ref{eq:M-tilde-K-isotropic} reveals that,
the magnetization $\tilde{\mathbf{M}}$ is parallel to
the inducing field $\tilde{\mathbf{H}}_{0}$
is parallel to an ellipsoid axis, the magnetization is 

If we additionally consider that $\chi << 1$, the magnetization
$\tilde{\mathbf{M}}$ (Eq. \ref{eq:M-tilde}) can be approximated
as follows:
\begin{equation}
\tilde{\mathbf{M}} \approx \chi \, \tilde{\mathbf{H}}_{0} \: ,
\label{eq:M-tilde-chi-H0}
\end{equation}
which is the classical equation describing the induced
magnetization in applied geophysics \citep[p. 87]{blakely1996}.
Notice that, in this particular case, the magnetization is
parallel to the inducing field $\tilde{\mathbf{H}}_{0}$.
...
Notice that the elements $\tilde{n}^{\ddagger}_{ij}$
(Eqs. \ref{eq:n-tilde-dagger-ii} and \ref{eq:n-tilde-dagger-ij}) 
are proportional to the second derivatives of the function
$\tilde{f}^{\ddagger}(\tilde{\mathbf{r}})$ (Eq. \ref{eq:fe-tilde}),
which is harmonic. Consequently, the diagonal elements defined
in Eq. \ref{eq:n-tilde-dagger-ii} satisfies the condition
...
A deduction of these formulas was presented by \citet{tejedor1995}.
Despite the small differences, these formulas can also be found in
\citet{clark1986}.
...
\section{Computational implementation and testing}


The code is implemented in Python language,
by using use the NumPy and SciPy libraries \citep{vanderwalt2011},
as part of the open-source source library
Fatiando a Terra \citep{uieda2013}.
Because of that, it is very modular and has a
...
freely reusable lessons of the Software Carpentry 
(\url{https://software-carpentry.org/}) and at the
description of the doctest module that is available
...
BSD 3-clause licence as part of the Fatiando a Terra 
