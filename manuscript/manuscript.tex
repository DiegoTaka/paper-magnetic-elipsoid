...
In general, the susceptibility tensor $\mathbf{K}$ is represented as
follows:
\begin{equation}
\mathbf{K} = \mathbf{U}
\left[ \begin{array}{ccc}
...
coincide with the ellipsoid axes, $\mathbf{U} = \mathbf{V}$
where $\mathbf{V}$ is defined by Eq. \ref{eq:V_triaxial_prolate} 
or \ref{eq:V_oblate}), depending on the ellipsoid type.
