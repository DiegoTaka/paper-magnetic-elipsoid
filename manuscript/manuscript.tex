...
(Fig. \ref{fig:orientation-angles}b), respectively, depending on the ellipsoid type.
These unit vectors depend on orientation angles $\alpha$, 
$\beta$, and $\gamma$, which are not commonly used in
geosciences. These angles, however, can be defined from
$\varepsilon$, $\zeta$, and $\eta$ (Fig. \ref{fig:orientation-angles}a)
...
\end{equation}
Similarly, the unit vectors defining the matrix $\mathbf{V}$ (Eq. \ref{eq:A}) for
prolate ellipsoids (i.e., $a > b = c$) are calculated
by using Eqs. \ref{eq:v1_triaxial_prolate}, \ref{eq:v2_triaxial_prolate}, and
\ref{eq:v3_triaxial_prolate}, but with $\gamma = 0^{\circ}$
\citep{emerson1985}.
Finally, the unit vectors defining the matrix $\mathbf{V}$ (Eq. \ref{eq:A}) for
oblate ellipsoids (i.e., $a < b = c$) are calculated as follows:
\begin{equation}
\mathbf{v}_{1} = \left[\begin{array}{c} 
...
in the main coordinate system (Fig. \ref{fig:orientation-angles}a), as follows:
\begin{equation}
\mathbf{K} = \mathbf{U}
\left[ \begin{array}{ccc}
k_{1} & 0 & 0 \\
