...
\end{equation}
where $\tilde{n}^{\dagger}_{33}$ is defined in the 
following sections, depending on the ellipsoid
type.
It is worth, however, to stress that $\tilde{n}^{\dagger}_{33}$ is
...
to define
\begin{equation}
\chi_{max} = \frac{\epsilon}{\tilde{n}^{\dagger}_{33}} \: ,
\label{eq:max-chi}
\end{equation}
...
has been using $\chi_{max} = 0.1$ SI as a limit value for
...
(Eqs. \ref{eq:M-K-isotropic} and \ref{eq:M-K-isotropic-inverse-Lambda}).
Equation \ref{eq:max-chi}, on the other hand, defines $\chi_{max}$ 
...
Eq. \ref{eq:delta-H} will be presented in
the following section.

The $\Delta \mathbf{H}(\mathbf{r})$ 
...
The elements of the transformed depolarization tensor 
$\tilde{\mathbf{N}}^{\dagger}$ used in the previous sections
...
\subsection{Relative error in neglecting the self-demagnetization}
