...
\begin{equation}
\mathbf{M} = \mathbf{K} \, \mathbf{H}^{\ast} \: ,
\label{eq:M-KH}
\end{equation}
where $\mathbf{H}^{\ast}$ is the resultant uniform magnetic field 
...
\begin{equation}
\mathbf{H}(\mathbf{r}) = \mathbf{H}_{0} 
- \mathbf{N}(\mathbf{r}) \, \mathbf{K} \, \mathbf{H}^{\ast} \: ,
...
It is convenient to use $\tilde{f}^{\ast}(\tilde{\mathbf{r}})$
and $\tilde{f}^{\dagger}(\tilde{\mathbf{r}})$ to define 
the function $\tilde{f}(\tilde{\mathbf{r}})$ evaluated, 
respectively, at points 
$\tilde{\mathbf{r}}$ inside and outside the volume $V$ of the
ellipsoidal body.

The scalar function $\tilde{f}^{\ast}(\tilde{\mathbf{r}})$
is given by
\begin{equation}
\tilde{f}^{\ast}(\tilde{\mathbf{r}}) = \pi \, abc \, 
\int_{0}^{\infty} \left( 1 
- \frac{\tilde{x}^{2}}{a^{2} + u} 
- \frac{\tilde{y}^{2}}{b^{2} + u}
...
In a similar way, the function $\tilde{f}^{\dagger}(\tilde{\mathbf{r}})$
is given by
\begin{equation}
...
_{\tilde{\mathbf{K}}} \; 
\underbrace{\mathbf{V}^{\top} \mathbf{H}^{\ast}}
_{\tilde{\mathbf{H}}^{\ast}} \: ,
\label{eq:H-tilde}
\end{equation}
...
point located within or without the volume $V$ of the
ellipsoidal body.
...
for defining the resultant uniform magnetic field $\tilde{\mathbf{H}}^{\ast}$
inside the ellipsoidal body as follows:
\begin{equation}
\tilde{\mathbf{H}}^{\ast} = 
\left( \mathbf{I} - \tilde{\mathbf{N}}^{\ast} \, \tilde{\mathbf{K}} \right)^{-1}
\tilde{\mathbf{H}}_{0} \: ,
\label{eq:Hi-tilde}
\end{equation}
where $\mathbf{I}$ is the identity matrix and 
$\tilde{\mathbf{N}}^{\ast}$ represents the
transformed depolarization tensor evaluated within 
the volume $V$ of the ellipsoidal body. The elements
of $\tilde{\mathbf{N}}^{\ast}$ will be presented in a 
latter section.

Let us pre-multiply the uniform internal field $\tilde{\mathbf{H}}^{\ast}$
...
\tilde{\mathbf{H}}_{0} \\
&=  
\left( \mathbf{I} - \tilde{\mathbf{K}} \, \tilde{\mathbf{N}}^{\ast} \right)^{-1}
...
- \tilde{\mathbf{N}}^{\dagger}(\tilde{\mathbf{r}}) \, \tilde{\mathbf{M}} \: .
\label{eq:delta-H-tilde}
\end{equation}
The elements of the transformed depolarization tensor 
$\tilde{\mathbf{N}}^{\dagger}(\tilde{\mathbf{r}})$ used in 
...
\subsubsection{Depolarization tensor $\tilde{\mathbf{N}}^{\ast}$}

The elements of the transformed depolarization tensor 
$\tilde{\mathbf{N}}^{\ast}$ used to compute the 
uniform magnetic field $\tilde{\mathbf{H}}^{\ast}$
...
$\tilde{f}^{\ast}(\tilde{\mathbf{r}})$ (Eq. \ref{eq:fi-tilde}).
As we have already pointed out, the  
$\tilde{f}^{\ast}(\tilde{\mathbf{r}})$ (Eq. \ref{eq:fi-tilde}) is a
quadratic function of the spatial coordinates $\tilde{x}$,
$\tilde{y}$ and $\tilde{z}$. Consequently, the off-diagonal
elements $\tilde{n}^{\ast}_{ij}(\tilde{\mathbf{r}})$, $i = 1, 2, 3$, 
$j = 1, 2, 3$, $i \ne j$, are equal to zero. The remaining
diagonal elements are given by \citep{stoner1945}:
\begin{equation}
\tilde{n}^{\ast}_{ii}(\tilde{\mathbf{r}}) = \frac{abc}{2}
\int_{0}^{\infty} \frac{1}{\left( e_{i}^{2} 
+ u \right) R(u)} \, du \: , \quad i = 1, 2, 3 \: ,
\label{eq:n-tilde-ast-ii}
\end{equation}
...
\begin{equation}
\mathbf{N}(\mathbf{r}) = 
\mathbf{V} \, \tilde{\mathbf{N}}^{\ast} \, 
\mathbf{V}^{\top} \: ,
\label{eq:N-V-N-ast-VT}
\end{equation}
where $\tilde{\mathbf{N}}^{\ast}$ (Eqs. \ref{eq:Hi-tilde}, 
...
volume $V$ of the ellipsoid, the elements 
$\tilde{n}^{\ast}_{ii}(\tilde{\mathbf{r}})$ (Eq. \ref{eq:n-tilde-ast-ii})
of $\tilde{\mathbf{N}}^{\ast}$ represent the eigenvalues while the 
columns of $\mathbf{V}$ represent the eigenvectors of the 
original depolarization tensor $\mathbf{N}(\mathbf{r})$.

INTRODUZIR OS ELEMENTOS $\tilde{n}^{\ast}_{ii}(\tilde{\mathbf{r}})$
PARA OS TRES TIPOS DE ELIPSOIDE

\subsubsection{Depolarization tensor $\tilde{\mathbf{N}}^{\dagger}(\tilde{\mathbf{r}})$}

The elements of the transformed depolarization tensor 
$\tilde{\mathbf{N}}^{\dagger}(\tilde{\mathbf{r}})$ used to compute
the magnetic field $\Delta \tilde{\mathbf{H}}(\tilde{\mathbf{r}})$ 
produced by the ellipsoid (Eq. \ref{eq:delta-H-tilde}) and
the total-field anomaly $\Delta \tilde{T}(\tilde{\mathbf{r}})$
(Eqs. \ref{eq:delta-T-tilde} and \ref{eq:delta-T-tilde-approx}) 
are calculated according to Equation \ref{eq:nij-tilde}, with 
$\tilde{f}(\tilde{\mathbf{r}})$ given by $\tilde{f}^{\dagger}(\tilde{\mathbf{r}})$
(Eq. \ref{eq:fe-tilde}).
