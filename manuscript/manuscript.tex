...
Another particularity of ellipsoids are that they are
...
axes of a body.
It is a function of the shape of the body and gives rise
to shape anisotropy \citep{thompson1986, clark1999, tauxe2003}.
...
orientation defined by angles \textit{strike}, \textit{dip}, and
\textit{rake} (Fig. \ref{fig:orientation-angles}a), and semi-axes defined by
positive constants $a$, $b$, $c$ (Fig. \ref{fig:orientation-angles}b).
The angles strike, dip and rake are commonly used to
...
For a given ellipsoid, the elements $\tilde{n}^{\dagger}_{ii}$, 
$i = 1, 2, 3$, of $\tilde{\mathbf{N}}^{\dagger}$ are constants 
which are expressed in terms of the ellipsoid semi-axes.
...
In this case, the magnetization $\tilde{\mathbf{M}}$ (Eq. \ref{eq:M-tilde-remanence})
can be rewritten as follows:
\begin{equation}
\tilde{\mathbf{M}} = \left[ \begin{array}{ccc}
\frac{1}{1 - \chi \, \tilde{n}^{\dagger}_{11}} & 0 & 0 \\
0 & \frac{1}{1 - \chi \, \tilde{n}^{\dagger}_{22}} & 0 \\
0 & 0 & \frac{1}{1 - \chi \, \tilde{n}^{\dagger}_{33}}
\end{array} \right] \left( \chi \, \tilde{\mathbf{H}}_{0} +
\tilde{\mathbf{M}}_{R} \right) \: .
\label{eq:M-tilde-K-isotropic}
\end{equation}
This equation is in perfect agreement with those presented 
by \citet[Eqs. ~13--15]{guo2001}.
The first term, depending on the inducing field $\tilde{\mathbf{H}}_{0}$, 
represents the induced magnetization whereas the term depending on
$\tilde{\mathbf{M}}_{R}$ is the remanent magnetization.
Equation \ref{eq:M-tilde-K-isotropic} reveals that, as pointed out by \citet{maxwell1873},
...
$\mathbf{M}$ (Eq. \ref{eq:M-KH}), defined in the main coordinate system,
can be approximated as follows:
\begin{equation}
\mathbf{M} \approx \chi \, \mathbf{H}_{0} +
\mathbf{M}_{R} \: ,
\label{eq:M-approx}
\end{equation}
...
is parallel to the inducing field $\mathbf{H}_{0}$,
...
Figure \ref{fig:figxxx} TEXT TEXT TEXT Fig. \ref{fig:figxxx} TEXT TEXT TEXT (Fig. \ref{fig:figxxx})
...
(a) Main coordinate system with axes $x$, $y$, and $z$.
The dark gray plane contains the center $(x_{c}, y_{c}, z_{c})$
...
which contains the $x$ and $y$ axes. The angle between "minus $x$"
and the strike direction is called \textit{strike}. The angle
between the horizontal plane and the dark gray plane
is called \textit{dip}. The line containing the unit vector
$\mathbf{u}$ defines the \textit{plunge direction}. The projection
of the plunge direction on the horizontal plane is called
\textit{plunge azimuth direction}. (b) Local coordinate system
with origin at the ellipsoid center $(x_{c}, y_{c}, z_{c})$
...
and oblate ellipsoids. Notice that, for triaxial and prolate ellipsoids,
